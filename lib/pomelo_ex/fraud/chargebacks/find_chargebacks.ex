defmodule PomeloEx.Fraud.Chargebacks.FindChargebacks do
  ...
  defp format_pair({:filter_transaction_id, value}),
    do: {"filter[transaction_id]", to_string(value)}

  defp format_pair({:filter_created_at_from, value}),
    do: {"filter[created_at][from]", to_string(value)}

  defp format_pair({:filter_created_at_to, value}),
    do: {"filter[created_at][to]", to_string(value)}

  defp format_pair({:filter_updated_at_from, value}),
    do: {"filter[updated_at][from]", to_string(value)}

  defp format_pair({:filter_updated_at_to, value}),
    do: {"filter[updated_at][to]", to_string(value)}
